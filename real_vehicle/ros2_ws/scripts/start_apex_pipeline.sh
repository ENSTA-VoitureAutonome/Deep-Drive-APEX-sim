...
FIXED_MAP_BUILDER_SCRIPT="${APEX_FIXED_MAP_BUILDER_SCRIPT:-/work/repo/real_vehicle/tools/core/build_fixed_map_from_offline_refined.py}"
...
RECOGNITION_TRACKER_REPLAN_REQUEST_TOPIC="${APEX_RECOGNITION_TRACKER_REPLAN_REQUEST_TOPIC:-}"
RECOGNITION_TRACKER_STUCK_RECOVERY_ENABLED="${APEX_RECOGNITION_TRACKER_STUCK_RECOVERY_ENABLED:-}"
RECOGNITION_TRACKER_STUCK_FORWARD_CMD_THRESHOLD_MPS="${APEX_RECOGNITION_TRACKER_STUCK_FORWARD_CMD_THRESHOLD_MPS:-}"
RECOGNITION_TRACKER_STUCK_MOTION_SPEED_THRESHOLD_MPS="${APEX_RECOGNITION_TRACKER_STUCK_MOTION_SPEED_THRESHOLD_MPS:-}"
RECOGNITION_TRACKER_STUCK_MOTION_DISTANCE_THRESHOLD_M="${APEX_RECOGNITION_TRACKER_STUCK_MOTION_DISTANCE_THRESHOLD_M:-}"
RECOGNITION_TRACKER_STUCK_DETECT_HOLD_S="${APEX_RECOGNITION_TRACKER_STUCK_DETECT_HOLD_S:-}"
RECOGNITION_TRACKER_STUCK_REVERSE_LINEAR_MPS="${APEX_RECOGNITION_TRACKER_STUCK_REVERSE_LINEAR_MPS:-}"
RECOGNITION_TRACKER_STUCK_REVERSE_ANGULAR_Z_RPS="${APEX_RECOGNITION_TRACKER_STUCK_REVERSE_ANGULAR_Z_RPS:-}"
RECOGNITION_TRACKER_STUCK_REVERSE_DURATION_S="${APEX_RECOGNITION_TRACKER_STUCK_REVERSE_DURATION_S:-}"
RECOGNITION_TRACKER_STUCK_REPLAN_WAIT_S="${APEX_RECOGNITION_TRACKER_STUCK_REPLAN_WAIT_S:-}"
RECOGNITION_TRACKER_STUCK_RECOVERY_COOLDOWN_S="${APEX_RECOGNITION_TRACKER_STUCK_RECOVERY_COOLDOWN_S:-}"
RECOGNITION_TRACKER_STUCK_RECOVERY_MAX_COUNT="${APEX_RECOGNITION_TRACKER_STUCK_RECOVERY_MAX_COUNT:-}"
...
if [[ "${ESTIMATION_BACKEND}" == "fixed_map" ]]; then
  ...
  FIXED_MAP_ROUTE_ODOM_TOPIC="${FIXED_MAP_ROUTE_ODOM_TOPIC:-${FUSION_ODOM_TOPIC}}"
  FIXED_MAP_ROUTE_REPLAN_REQUEST_TOPIC="${FIXED_MAP_ROUTE_REPLAN_REQUEST_TOPIC:-/apex/planning/fixed_map_replan_request}"
  RECOGNITION_TRACKER_REPLAN_REQUEST_TOPIC="${RECOGNITION_TRACKER_REPLAN_REQUEST_TOPIC:-${FIXED_MAP_ROUTE_REPLAN_REQUEST_TOPIC}}"
fi
...
if [[ "${ENABLE_OFFLINE_SUBMAP_REFINER}" == "1" ]]; then
  ...
  if [[ -f "${OFFLINE_REFINER_SCRIPT}" ]]; then
    ...
    python3 "${OFFLINE_REFINER_SCRIPT}" \
      ...
      -p "save_output_dir:=${APEX_OFFLINE_REFINER_SAVE_OUTPUT_DIR:-/work/repo/real_vehicle/.apex_runtime/offline_refined_maps}" &
...
if [[ "${ENABLE_FIXED_MAP_PUBLISHER}" == "1" ]]; then
  ...
  FIXED_MAP_PUBLISHER_SCRIPT="${APEX_FIXED_MAP_PUBLISHER_SCRIPT:-/work/repo/real_vehicle/tools/core/apex_fixed_map_publisher.py}"
...
if [[ "${ENABLE_FIXED_MAP_ROUTE_PLANNER}" == "1" ]]; then
  ...
  python3 -m apex_telemetry.perception.fixed_map_route_planner_node \
    ...
    -p "odom_topic:=${FIXED_MAP_ROUTE_ODOM_TOPIC:-/apex/odometry/fixed_map_localized}" \
    -p "replan_request_topic:=${FIXED_MAP_ROUTE_REPLAN_REQUEST_TOPIC:-/apex/planning/fixed_map_replan_request}" \
...
if [[ "${ENABLE_RECOGNITION_TOUR_TRACKER}" == "1" ]]; then
  ...
  if [[ -n "${RECOGNITION_TRACKER_REPLAN_REQUEST_TOPIC}" ]]; then
    TRACKER_CMD+=(-p "replan_request_topic:=${RECOGNITION_TRACKER_REPLAN_REQUEST_TOPIC}")
  fi
  if [[ -n "${RECOGNITION_TRACKER_STUCK_RECOVERY_ENABLED}" ]]; then
    TRACKER_CMD+=(-p "stuck_recovery_enabled:=$(normalize_bool_override "${RECOGNITION_TRACKER_STUCK_RECOVERY_ENABLED}")")
  fi
  if [[ -n "${RECOGNITION_TRACKER_STUCK_FORWARD_CMD_THRESHOLD_MPS}" ]]; then
    TRACKER_CMD+=(-p "stuck_forward_cmd_threshold_mps:=${RECOGNITION_TRACKER_STUCK_FORWARD_CMD_THRESHOLD_MPS}")
  fi
  if [[ -n "${RECOGNITION_TRACKER_STUCK_MOTION_SPEED_THRESHOLD_MPS}" ]]; then
    TRACKER_CMD+=(-p "stuck_motion_speed_threshold_mps:=${RECOGNITION_TRACKER_STUCK_MOTION_SPEED_THRESHOLD_MPS}")
  fi
  if [[ -n "${RECOGNITION_TRACKER_STUCK_MOTION_DISTANCE_THRESHOLD_M}" ]]; then
    TRACKER_CMD+=(-p "stuck_motion_distance_threshold_m:=${RECOGNITION_TRACKER_STUCK_MOTION_DISTANCE_THRESHOLD_M}")
  fi
  if [[ -n "${RECOGNITION_TRACKER_STUCK_DETECT_HOLD_S}" ]]; then
    TRACKER_CMD+=(-p "stuck_detect_hold_s:=${RECOGNITION_TRACKER_STUCK_DETECT_HOLD_S}")
  fi
  if [[ -n "${RECOGNITION_TRACKER_STUCK_REVERSE_LINEAR_MPS}" ]]; then
    TRACKER_CMD+=(-p "stuck_reverse_linear_mps:=${RECOGNITION_TRACKER_STUCK_REVERSE_LINEAR_MPS}")
  fi
  if [[ -n "${RECOGNITION_TRACKER_STUCK_REVERSE_ANGULAR_Z_RPS}" ]]; then
    TRACKER_CMD+=(-p "stuck_reverse_angular_z_rps:=${RECOGNITION_TRACKER_STUCK_REVERSE_ANGULAR_Z_RPS}")
  fi
  if [[ -n "${RECOGNITION_TRACKER_STUCK_REVERSE_DURATION_S}" ]]; then
    TRACKER_CMD+=(-p "stuck_reverse_duration_s:=${RECOGNITION_TRACKER_STUCK_REVERSE_DURATION_S}")
  fi
  if [[ -n "${RECOGNITION_TRACKER_STUCK_REPLAN_WAIT_S}" ]]; then
    TRACKER_CMD+=(-p "stuck_replan_wait_s:=${RECOGNITION_TRACKER_STUCK_REPLAN_WAIT_S}")
  fi
  if [[ -n "${RECOGNITION_TRACKER_STUCK_RECOVERY_COOLDOWN_S}" ]]; then
    TRACKER_CMD+=(-p "stuck_recovery_cooldown_s:=${RECOGNITION_TRACKER_STUCK_RECOVERY_COOLDOWN_S}")
  fi
  if [[ -n "${RECOGNITION_TRACKER_STUCK_RECOVERY_MAX_COUNT}" ]]; then
    TRACKER_CMD+=(-p "stuck_recovery_max_count:=${RECOGNITION_TRACKER_STUCK_RECOVERY_MAX_COUNT}")
  fi
  "${TRACKER_CMD[@]}" &
